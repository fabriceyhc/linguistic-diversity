...
def load_pretrained_encdec(device: str = 'cuda'):
    """Load the original pretrained encoder-decoder (no additional pretraining)."""
    from model_loader import load_model
...
def evaluate_encdec_on_tasks(
    ...
    try:
        if use_pretrained:
            tokenizer, model = load_pretrained_encdec(device)
